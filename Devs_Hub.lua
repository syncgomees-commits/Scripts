...
local FlightTog = MainTab:AddToggle("Fly Hack", "fly_hack", function(v)
    ...
    if IsAndroid then
        ...
        if v then
            Xan.Notify({Title = "Flight", Content = "Toque a tela para se mover, use bot̵es para subir/descer", Type = "Info"})
        end
    end
        ...
        if not IsAndroid then
            ...
            if #touchPositions > 0 then
                for _, touchPos in pairs(touchPositions) do
                    ...
                    local touchX = touchPos.X - centerX
                    local touchY = touchPos.Y - centerY
                    
                    local magnitude = math.sqrt(touchX * touchX + touchY * touchY)
                    
                    --// Zona morta menor para movimento mais responsivo
                    if magnitude > 30 then
                        local normalizedX = touchX / magnitude
                        local normalizedY = touchY / magnitude
                        
                        --// RIGHT/LEFT
                        if math.abs(normalizedX) > 0.2 then
                            dir = dir + Camera.CFrame.RightVector * normalizedX
                        end
                        
                        --// FORWARD/BACKWARD
                        if math.abs(normalizedY) > 0.2 then
                            dir = dir - Camera.CFrame.LookVector * normalizedY
                        end
                    end
                end
            end
        ...
        elseif name == "UpBtn" then
            ...
            btn.MouseButton1Down:Connect(function()
                if Config.Fly and BodyVel then
                    ...
                    BodyVel.Velocity = BodyVel.Velocity + Vector3.new(0, Config.FlySpeed, 0)
                end
            end)
...
Xan.Notify({Title = "DEVS HUB", Content = "Loaded successfully!", Type = "Success"})    WalkSpeed = 16,
    JumpPower = 50,
    TargetPlr = nil,
    AimRange = 1000,
    HitParts = {Head = true, Chest = true},
...
local function CreateAndroidButtons()
    ...
    local buttonConfigs = {
        ...
        {Name = "EspBtn", Pos = UDim2.new(0, 10, 0.7, -25), Color = Color3.fromRGB(255, 200, 0)},
        {Name = "TeleBtn", Pos = UDim2.new(0.9, -50, 0.5, -25), Color = Color3.fromRGB(150, 0, 255)},
        {Name = "UpBtn", Pos = UDim2.new(0.9, -80, 0.3, -25), Color = Color3.fromRGB(100, 200, 100), Hidden = true, Size = 60},
        {Name = "DownBtn", Pos = UDim2.new(0.9, -80, 0.65, -25), Color = Color3.fromRGB(200, 100, 100), Hidden = true, Size = 60},
    }
...
local FlightTog = MainTab:AddToggle("Fly Hack", "fly_hack", function(v)
    ...
    if IsAndroid then
        ...
        if v then
            Xan.Notify({Title = "Flight", Content = "Use joystick to move, buttons to ascend/descend", Type = "Info"})
        end
    end
    if not v and LocalPlayer.Character then
        LocalPlayer.Character.Humanoid.PlatformStand = false
            ...
            if #touchPositions > 0 then
                for _, touchPos in pairs(touchPositions) do
                    ...
                    if magnitude > 50 then
                        ...
                        --// RIGHT/LEFT
                        dir = dir + Camera.CFrame.RightVector * normalizedX * 0.5
                        
                        --// FORWARD/BACKWARD
                        dir = dir - Camera.CFrame.LookVector * normalizedY * 0.5
                    end
                end
            end
        end
