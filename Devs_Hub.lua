...
Connections["Loop"] = RunService.Stepped:Connect(function()
    ...
    if Config.Fly then
        ...
        if not IsAndroid then
            ...
            -- ANDROID: Use Humanoid.MoveDirection (native analog) + touch fallback + up/down buttons
            if hum and hum.MoveDirection and hum.MoveDirection.Magnitude > 0 then
                -- Use native analog stick direction from Humanoid
                moveDirection = hum.MoveDirection
            else
                -- Fallback: interpret touch positions as analog input
